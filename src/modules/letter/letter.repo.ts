...
export class LetterRepo {
  ...
  async list(query: ListLetterQueryType, userId: number, isAdmin: boolean = false) {
    ...
    const shouldFilterByUserId = isAdmin ? false : (query.filterByUserId ?? true)
    const { filterByUserId: _, ...pagination } = query

    // Debug log (remove in production)
    console.log(
      'DEBUG - Raw filterByUserId from query:',
      query.filterByUserId,
      'type:',
      typeof query.filterByUserId
    )
    console.log(
      'DEBUG - Processed shouldFilterByUserId:',
    ...
    if (whereClause.status && !validStatuses.includes(whereClause.status as string)) {
      throw new BadRequestException(
        `Invalid status: ${whereClause.status}. Valid values are: ${validStatuses.join(', ')}`
      )
    }
