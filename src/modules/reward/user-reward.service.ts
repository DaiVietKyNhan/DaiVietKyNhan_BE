...
export class UserRewardService {
  ...
  async exchangeReward({ userId, rewardId }: { userId: number; rewardId: number }) {
    try {
      ...
      if (existingUserReward && existingUserReward.status === 'COMPLETED') {
        ...
        const codeToSave =
          reward.type === 'CODE' ? `CODE_${Date.now()}` : existingUserReward.code || null
    ...
    try {
      ...
      const existingHistorySet = new Set(
        existingHistories.map(
          (h) => `${h.userId}-${h.rewardId}-${h.exchangedAt?.getTime()}`
        )
      )
  ...
  async getSpecialUserRewards(query: PaginationQueryType) {
    const rewards = await this.rewardRepo.getSpecial5200()
    if (!rewards) {
      throw NotFoundRecordException
    }

    // Lấy danh sách UserRewardHistory với pagination
    const skip = (query.currentPage - 1) * query.pageSize
    const take = query.pageSize
    console.log('reward:', rewards.id)

    const [total, data] = await Promise.all([
      this.prismaService.userRewardHistory.count({
        where: {
          rewardId: rewards.id,
          status: 'CLAIMED',
          deletedAt: null
      ...
      this.prismaService.userRewardHistory.findMany({
        where: {
          rewardId: rewards.id,
          status: 'CLAIMED',
          deletedAt: null
